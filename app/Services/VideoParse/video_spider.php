...
class Video {
        ...
        if ($arr['status_code']==0) {
            $arr = ['code' => 200,
                ...
                'data' => [
                    ...
                    'musicurl' => property_exists( (object)$arr['aweme_detail'], 'music') ? $arr['aweme_detail']['music']['play_url']['url_list'][0] : '',
                    'music' => property_exists( (object)$arr['aweme_detail'], 'music') ? [
    ...
    public function weishi($url) {
        $loc = get_headers($url, true) ['Location'];
        Log::info("Location:".json_encode($loc));
        preg_match('/id=(.*)&spid/', $loc[0], $id);
        Log::info("vid:".json_encode($id));
        $arr = json_decode($this->curl('https://h5.weishi.qq.com/webapp/json/weishi/WSH5GetPlayPage?feedid=' . $id[1]), true);
        Log::info("微视解析:".json_encode($arr));
        ...
        if ($arr) {
            ...
            $arr = [
                ...
                'data' => [
                    ...
                    'avatar' => 'http:'.$arr['data']['Component_Play_Playinfo']['avatar'],
                    'time' => $arr['data']['Component_Play_Playinfo']['real_date'],
                    'title' => $arr['data']['Component_Play_Playinfo']['title'],
                    'cover' => 'http:'.$arr['data']['Component_Play_Playinfo']['cover_image'],
                    'url' => 'http:'.$video_url,
                    'preview' => config('app.preview')
                ]
            ];
    ...
    public function bilibili($url) {
        $locs1 = get_headers($url, true) ['Location'][0];
        Log::info("bilibili-locs1:".json_encode($locs1));
        $text1 = $this->curl($locs1);
        Log::info("bilibili-text1:".json_encode($text1));
        preg_match('/a href=\"(.*?)\"/', $text1, $locs2);
        Log::info("bilibili-locs2:".json_encode($locs2[1]));
        return $this->bbq($locs2[1]);
    }

    public function bbq($url) {
        $text = $this->curl($url);
        preg_match('/\"readyVideoUrl\":\"(.*?)\"/', $text, $video);
        Log::info("bilibili-text:".json_encode($text));
        Log::info("bilibili-video:".json_encode($video));
        preg_match('/\"readyPoster\":\"(.*?)\"/', $text, $cover);
        Log::info("bilibili-cover:".json_encode($cover));
        if ($video) {
            $arr = [
                'code' => 200,
                'msg' => '解析成功',
...
//         curl_close($curl);
//         Log::info("快手解析结果:".json_encode($result));
//         //获取到视频相关数据
...
//     }
    ...
    public function kuaishou($url) {
        ...
        Log::info("快手解析:{$locs}");
        preg_match('/photoId=(.*?)\&/', $locs, $matches);
        $headers = array('Cookie: did=web_',
            'Referer: ' . $locs, 'Content-Type: application/json');
        Log::info("快手解析matches:".json_encode($matches));
        $post_data = '{"photoId": "' . str_replace(['video/', '?'], '', $matches[1]) . '","isLongVideo": false}';
        Log::info("快手request body:$post_data");
        Log::info("快手header:".json_encode($headers));
        $curl = curl_init();
        ...
        if ($json) {
            $arr = [
                ...
                'data' => [
                    ...
                    'music' => [
                        'url' => $json['photo']['soundTrack']['audioUrls'][0]['url']
                    ],
                    'preview' => config('app.preview')
                ]
            ];
            Log::info("快手解析结果:".json_encode($arr));
            return $arr;
        }
    }
